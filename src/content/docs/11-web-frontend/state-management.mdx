...
For a store with `n` subscribers, evaluating every selector is O(n). If the store maintains a dependency index that identifies subscribers affected by the changed state slice, notification and render work can approach O(k), where `k` is the number whose selected slice changed; without that index, finding those `k` subscribers still costs O(n). The real ceiling is render work: 100 cheap subscribers can be fine, while one subscriber that reconstructs a 10,000-row table on every keystroke is not.
