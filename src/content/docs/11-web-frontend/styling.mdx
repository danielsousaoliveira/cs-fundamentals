...
Styling is constraint management: semantic markup provides meaning, layout primitives express relationships, and tokens prevent drift. The caveat is that CSS bugs often come from an invisible cascade, containing block, or stacking context, so the fix is to inspect the constraint, not increase specificity blindly.
