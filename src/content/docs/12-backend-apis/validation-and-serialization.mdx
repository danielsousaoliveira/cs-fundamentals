...
The schema does three things at once: it rejects shapes it doesn't
recognize (an unexpected `isAdmin: true` field is dropped, not silently
trusted — `.strict()` would reject it outright instead), it coerces to the
right type, and it bounds the range (`.max(120)`, `.max(5)`). `roles` only
...
request that a 10 KB `.max()` bound would have rejected at the boundary for
the cost of a length comparison.

**Ceiling and what happens at it.** A schema validation failure is a `422`
with field-level errors — cheap, synchronous, and informative. The
alternative — no schema, relying on the database's constraints to catch
bad data — turns the same bad input into a `500` from a constraint
violation, with a stack trace instead of a field name, discovered by
whoever's paging on database errors instead of by the client that sent it.
...
Stripe's API returns structured `422` errors with a `param` field naming
the exact offending key (`"param": "email"`) — schema validation surfaced
directly as API contract, not an internal implementation detail. Postgres
...
updates or, if fields are made optional to fix that, silently allows a
partial update to unset a field that should never be nullable (making every
field optional loses the "required on create" constraint). The fix is a
separate `.partial()` schema for updates that still enforces per-field
constraints on whichever fields are present.
