...
a set of correlated spans across process boundaries if you had
distributed tracing wired up on the call path — trace context propagated
on the call, so every hop's span nests under the same trace and reads as
one continuous timeline — and a request ID that ties every downstream
...
- **An event log to reconstruct "what happened."** Because no single
  service holds the full sequence in choreography, the append-only stream
  of events becomes the artifact you replay to answer "what happened to
  order 4471," rather than any one service's database. A broker's own
  retained topic is not automatically this artifact, though — brokers
  retain messages for operational reasons (replay after an outage, a
  finite window for slow consumers), not for audit, and their retention
  period, compaction rules, and access model are usually tuned for
  throughput, not for "prove to an auditor what happened eighteen months
  ago." A business or compliance audit trail — one with guaranteed
  retention, immutability, and access controls independent of the
  broker's operational settings — is normally a separate durable store
  the events are also written to, not a rebranding of whatever the broker
...
central event log — typically a dedicated store with its own retention
and durability guarantees, not just whatever window the broker happens
to retain — specifically so "what happened to this order" is answerable
without reconstructing state from five databases. Financial ledgers and
audit-heavy domains lean on event sourcing — the event stream *is* the
record of truth, held in a purpose-built event store, and current state
is a projection of it — precisely because the traceability that a
...
traceability — a synchronous call's return value and a correlated trace
across the call chain told you what happened for free, and an event
fired into a broker doesn't. You get
