...
**Health checks and ejection.** The check that decides whether a backend
receives traffic is a **readiness check**, not a liveness check —
liveness answers "is this process making progress, or should the
orchestrator restart it," readiness answers "can this specific instance
correctly handle a request right now," and a load balancer only cares
about the second question. A readiness probe is typically an HTTP `GET
/healthz` on an interval (5–10s is common), the backend ejected after N
consecutive failures (2–3 is typical) to avoid ejecting on one transient
blip, and re-admitted after M consecutive successes once it recovers. The
readiness *condition* is the part worth deriving rather than defaulting: a
shallow check (does the process answer at all) catches a crashed process
but not a process that's up and answering while its database connection
pool is exhausted — a **deep readiness check** that verifies the
dependency the backend actually needs (a fast query against its database,
not just a 200 from the process) catches the second case, at the cost of
the check itself now being able to fail for a reason unrelated to that
specific backend instance (the shared database being down takes every
backend's deep check down with it, ejecting the entire fleet at once — a
shallow check would have correctly left them all in rotation, unable to
help either way).
...
backends, one running at half the throughput of the other two — call each
healthy backend's capacity `C`, so the fleet's combined capacity is `2.5C`
(2 backends at `C`, 1 at `0.5C`). Round robin still sends the degraded
backend a flat third of incoming requests regardless of whether it can
clear that share. At a third of `C` — that is, whenever the fleet's total
arrival rate `λ` stays at or below `1.5C` — the degraded backend can still
keep up with its fixed third and nothing backs up. Only once `λ` exceeds
`1.5C` does that backend's assigned share (`λ/3`) exceed what it can
actually clear (`0.5C`), and only then does its queue start to grow and
its latency degrade — round robin's failure here is conditional on load,
not automatic. Least connections doesn't wait for that threshold: it
routes each new request to whichever backend currently has the fewest
active requests, so as the degraded backend's in-flight count climbs
relative to its peers, new requests get steered away from it — its share
of *new* requests falls below a third *before* a hard overload point is
reached, not just after. That's a live, continuous response to the
observed queue, not a guarantee that the degraded backend's share lands
exactly at the throughput it can clear.
...
- **Don't run only shallow health checks on a backend with a hard
  ...
  pool passes every shallow readiness check while actively serving errors
  — the gap a deep check exists to close.
- **Don't run only deep health checks either, without considering the
  ...
  instance). The right answer is usually both: a lightweight liveness
  probe (is the process itself alive and worth restarting if not) kept
  entirely separate from the readiness signal that actually gates traffic
  admission, so a shared-dependency blip degrades rotation without
  triggering process restarts too.
...
Core Envoy's cluster default is round robin (`ROUND_ROBIN`); Least Request
— a variant of least-connections generalized with power-of-two-choices
sampling to avoid the herd problem noted above — is available and has to
be configured explicitly. Envoy Gateway, the Kubernetes Gateway API
implementation built on Envoy, flips that default and ships with Least
Request out of the box, precisely because a service-mesh-style deployment
is where backend response times vary continuously with real workload skew,
not just at failure boundaries — round robin is a worse default for that
environment specifically, which is why the more specialized project
changed it rather than core Envoy changing its general-purpose default.
AWS's Application
...
Fix: separate the liveness probe (is this process alive and worth
restarting if not) from the readiness check that gates traffic and is
tied to the specific rollout condition, and stage the shared-resource
change so it doesn't create a moment where every deep readiness check
fails at once. Detect it earlier by testing the health-check behavior
specifically during the deploy sequence, not just at steady state.
...
fleet on a database blip.** — Run two checks: a shallow readiness check
