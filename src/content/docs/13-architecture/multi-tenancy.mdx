...
- **Row-level security (RLS).** The same shared table, but the database
  ...
  remembered to add a `WHERE tenant_id = ?` clause:
  ...
  data breach into a visible bug.
...
3. A product currently runs shared-table-with-RLS for 500 tenants. An
   enterprise prospect's security review requires "logical isolation
   demonstrable in an audit." Which isolation tier satisfies that
   requirement, and what does adopting it cost operationally that RLS
   didn't?
