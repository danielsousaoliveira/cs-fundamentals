...
1–5ms same-region, more with retries or a slow downstream. What that
compounds into depends on the shape of the fan-out. If the 3 (or 15)
calls are made **sequentially** — each one waiting on the last — the
added latency is roughly additive: 3 in-process calls turned into 3
sequential service calls adds single-digit milliseconds; 15 sequential
calls adds tens of milliseconds, straightforwardly. If they're made **in
parallel**, the request's latency is governed by whichever call finishes
last, not the sum — but "governed by the slowest" is a statement about
probability, not a guarantee equal to any one call's own p99. With 15
independent downstream calls each individually hitting their stated p99
1% of the time, the chance that *at least one* of the 15 exceeds its own
p99 on a given request is roughly `1 - 0.99^15 ≈ 14%` — meaningfully
higher than 1%, and higher again the more calls are fanned out in
parallel. Decomposing a single logical operation into many downstream
calls, sequential or parallel, raises the odds that *some* dependency
having a bad moment becomes *this* request having a bad moment — the
mechanism is compounding probability across dependencies, not a single
slow call setting a hard ceiling.
...
failure — the saga pattern was needed and wasn't built. Fix: implement an
explicit saga — compensating actions for each step (cancel the order if
the inventory step fails), and something tracking which steps have
completed so a partial failure knows what to undo. An event-driven design
with retryable, idempotent steps is a reasonable way to *implement* that
saga's steps, but it doesn't substitute for having the compensating
actions and the workflow state in the first place — idempotency makes a
retry safe, it doesn't make an unwound half-completed operation
consistent on its own. Detect it earlier by writing the failure-mode
