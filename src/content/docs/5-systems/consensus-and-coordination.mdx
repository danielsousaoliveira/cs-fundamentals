...
have seen both. The overlap alone doesn't produce safety — a protocol still
has to make the overlapping node *refuse* to endorse a second, conflicting
value once it has already committed to the first. Raft does this with
explicit rules: one vote per term, and a log entry only becomes committed
once it's replicated to a majority *and* matches the leader's log up to that
point — so the overlap node's own persisted state is what blocks it from
agreeing to both. Majority overlap is what makes that refusal possible;
it's the protocol's state and voting rules that make it happen.
...
async function writeWithFencingToken(resourceId: string, token: number, data: unknown) {
...
atomic-broadcast protocol, not Paxos or Raft) still coordinates HBase and
legacy Kafka deployments; modern Kafka runs its own Raft-based KRaft
controllers instead and no longer needs ZooKeeper at all. Consul (Raft)
provides the same leader-election and coordination primitives for
service-discovery setups. These are named here as what to reach for, not
compared feature-by-feature — the point is knowing what a consensus-backed
coordination service guarantees that a plain cache cannot.
...
**Symptom: a consensus cluster stopped accepting writes after losing three
nodes out of five.** This is the majority rule working as designed, not a
bug — three failures out of five leaves only two survivors, short of the
three needed for a majority, so the survivors correctly refuse to proceed
rather than risk a split decision. (Losing two out of five, by contrast,
still leaves three survivors — a majority — and the cluster keeps accepting
writes; it's only a *third* member going down, or being partitioned away on
top of the first two, that crosses the line.) The failure mode is mistaking
this for an outage to page on immediately, rather than recognizing it as the
fault-tolerance boundary the cluster size was chosen for.
