...
Raft and Paxos (Raft's more implementable, better-documented descendant) both
guarantee: **at most one value is chosen per decision, and once chosen, it
...
- A write to a Raft/Paxos cluster commits only after a majority acknowledges
  it — so commit latency is bounded by the *median* node's response time
  among a majority, not the fastest node. A 5-node cluster with one slow
  node still commits fast, because the slow node isn't needed for the
  majority; a 5-node cluster with two slow nodes is now waiting on a
  borderline case.
...
predictably as members are lost. ZooKeeper (a Paxos-family protocol) and
Consul provide the same leader-election and coordination primitives for
