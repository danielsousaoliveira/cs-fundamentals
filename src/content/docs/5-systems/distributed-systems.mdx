...
- **At-most-once** — send once, don't retry. Simple, but a lost *request*
  means the effect never happens; a lost *response* means the effect
  happened but the caller has no way to know it did.
...
async function chargeCard(idempotencyKey: string, amount: number) {
  // Reserve the key first, with a unique constraint on idempotencyKey.
  // A concurrent retry's insert fails here instead of both racing past
  // the read below and both calling the gateway.
  const reserved = await db.charges
    .insert({ key: idempotencyKey, status: 'pending' })
    .onConflict('key')
    .ignore();

  if (!reserved) {
    return db.charges.findByKey(idempotencyKey); // another attempt owns it
  }

  // The gateway call itself must be passed the same idempotency key, so
  // that a retry after a crash here — before the row below is written —
  // lands on the gateway's own dedup rather than charging twice.
  const charge = await paymentGateway.charge(amount, idempotencyKey);
  await db.charges.update({ key: idempotencyKey }, { status: 'done', ...charge });
  return charge;
}
```

Two things make this safe where the naive read-then-write version isn't.
First, the row is reserved with a database-enforced unique constraint before
any external call happens, so two concurrent retries can't both pass a
check and both charge the card — one wins the insert, the other reads its
result, which is only complete once the winner finishes; a caller that reads
back a `pending` row needs to poll or wait rather than treat it as done, and
a winner that crashes before finishing leaves a `pending` row that needs its
own timeout-and-recovery sweep, not an indefinite wait. Second, the gateway
call itself carries the idempotency key, so if
the process crashes *after* the gateway charges the card but *before* the
local row is marked done, a subsequent retry still hits the gateway's own
idempotency guarantee instead of creating a second charge — the local record
alone can't cover that gap, because a retry with no stored result looks
identical to a request that never ran.

The idempotency key is what turns "retry blindly" into a safe default, but
only when it's enforced with a real constraint end to end — key check plus
key insert as one atomic step, and the same key passed through to any
downstream call that isn't itself transactional with the local write. Most
production incidents attributed to "the network is flaky" are actually a
check-then-act race or a key that stopped at the local database instead of
reaching the external call.
...
`max(local, received) + 1`. It gives a **partial order that respects
causality** — if A caused B, A's clock value is smaller than B's — without
needing synchronized wall-clock time anywhere. It does not, on its own, give
a total order: two genuinely concurrent events (neither caused the other)
can end up with the same or an arbitrarily-ordered clock value. Getting a
total order out of it requires an explicit, deterministic tie-breaker on top
— commonly the node ID appended to the counter — which is what turns "smaller
clock value" into a total order without claiming a causal relationship that
...
- A system with M services, each with 99.9% availability, calling each other
  ...
  99.9% each is 99.5% overall. The unavailability, which is what an SLA
  actually budgets, goes from 0.1% for one hop to about 0.5% for five — roughly
  5x worse, not 10x, but still enough to blow a tight availability target
  with a chain most people wouldn't think twice about. This is the
  arithmetic that justifies caching and async processing over long
  synchronous call chains, and it only gets worse as M grows.
...
gracefully. [Resilience patterns](/cs-fundamentals/13-architecture/resilience-patterns/)
covers the other half: what to put around the calls that have to stay
synchronous — retries, circuit breakers, timeouts, bulkheads.
