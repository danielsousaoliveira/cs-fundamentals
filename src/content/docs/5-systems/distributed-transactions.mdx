...
async function bookAndPay(orderId: string) {
  ...
  try {
    ...
    await notification.sendConfirmation(orderId);
    // no compensating action registered — see below
  } catch (err) {
...
`inventory.release` and `payment.refund` are honest compensations — they
return the system to a state indistinguishable from the step never having
happened. `notification.sendConfirmation` is not, and this is the case the
...
1. **Order the saga so irreversible steps happen last**, after every step
   that *can* fail has already succeeded. `bookAndPay` above does this
   correctly — the email goes out only once the booking and charge are both
   durable, so there's nothing left to fail that would require unsending it.
...
- **A saga's cost is an inconsistency window, bounded by how long the saga
  takes to either complete or fully compensate** — typically milliseconds to
  seconds, not indefinite. That's the trade: bounded, visible inconsistency
  instead of unbounded, invisible blocking.
