...
flowchart LR
  ...
  App --> DB[(Primary DB — writes + cold reads)]
...
assume ~10,000 active keys, so ~5 req/sec/key on average, well under the
100/minute cap for most, meaning the limiter must handle bursts, not steady
state.

Shape: a sliding-window counter per key in Redis (`INCR` + `EXPIRE`,
[Caching](/cs-fundamentals/5-systems/caching/)'s pattern), checked before the
request reaches the backend. At 50,000 req/sec, Redis's single-threaded
command execution handles this comfortably — each `INCR` is O(1) and
sub-millisecond.
